...
//		Map<String, String> transversalMap = new HashMap<String, String>();
	...
	protected void saveEntityToSendApp(String entityReference) {
			...
			log.debug("Save entity ref. to ENTITY_TO_PUSH table");
			...
			try {
					...
					if (!success) 
					{
						log.warn("Save entity ref. to table: failed");
					}
					connection.commit();
			...
			{
				log.warn("Save entity ref. to table: failed: " + t);
			}

			log.debug("Save entity ref. to table: done");

	}

	protected void deleteEntityToSendApp(String entityReference) {

		log.debug("Delete entity ref. from ENTITY_TO_PUSH table");
		String tableName = "ENTITY_TO_PUSH";
		...
		try {
			...
			if (!success) 
			{
				log.warn("Delete entity ref. from table: failed");
			}
			connection.commit();
			connection.setAutoCommit(wasCommit);
			m_sqlService.returnConnection(connection);
		...
		{
			log.warn("Delete entity ref. from table: failed: " + t);
		}

		log.debug("Delete entity ref. from table: done");
	}
